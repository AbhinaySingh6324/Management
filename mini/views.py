...
from .models import STUDENT_REGISTER, WORKER_REGISTER1, REGISTRATIONS, LOGIN_DETAILS, ALL_PROBLEMS1, TEMP_PROBLEMS, TEMP_REPORT,ADMIN_LOG
...
def login(request):
    ...
    if request.method=='POST':
        ...
        if id!=0:
            ...
            login_details.save()
        user = authenticate(email=em_id, password=ps)
        if user is not None:
            if user.is_active:
                login(request, user)
...
def student_register(request):
    ...
    if request.method=='POST':
        ...
        common.save()
        messages.success(request,"Got You Homie")
...
def detect(request):
    ...
    name='C:/Users/abhin/Desktop/Enviromentd/mini_project/'+str(prob['pic'])
